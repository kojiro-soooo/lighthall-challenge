...
function App() {
    ...
    useEffect(() => {
      ...
      .then((res)=>{ 
        // const coordinates = [];
        for (let i=0; i<2; i++){
          setCoordinates(arr => [...arr, {position: {lat: parseFloat(res[i].latitude), lng: parseFloat(res[i].longitude)}}])
    ...
    return (
      ...
      >
        <GoogleMap
          ...
          options={options}
        >
          {coordinates.map((item, index)=><MarkerF 
            key={index} 
          ...
          {selectedCenter && <InfoWindowF
          position={selectedCenter}
          >
            <div>{selectedCenter.lat}</div>
              </InfoWindowF>}
        </GoogleMap>
      </LoadScript>
    )
